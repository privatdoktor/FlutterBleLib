...
import hu.privatdoktor.multiplatformbleadapter.errors.BleErrorUtils;
import hu.privatdoktor.multiplatformbleadapter.errors.ErrorConverter;
import hu.privatdoktor.multiplatformbleadapter.exceptions.CannotMonitorCharacteristicException;
...
import hu.privatdoktor.multiplatformbleadapter.utils.DisposableMap;
import hu.privatdoktor.multiplatformbleadapter.utils.IdGenerator;
import hu.privatdoktor.multiplatformbleadapter.utils.LogLevel;
import hu.privatdoktor.multiplatformbleadapter.utils.RefreshGattCustomOperation;
import hu.privatdoktor.multiplatformbleadapter.utils.SafeExecutor;
import hu.privatdoktor.multiplatformbleadapter.utils.ServiceFactory;
import hu.privatdoktor.multiplatformbleadapter.utils.UUIDConverter;
import hu.privatdoktor.multiplatformbleadapter.utils.mapper.RxBleDeviceToDeviceMapper;
import hu.privatdoktor.multiplatformbleadapter.utils.mapper.RxScanResultToScanResultMapper;
...
public class BleAdapter {
    ...
    @Constants.BluetoothState
    public String getCurrentState() {
        if (bluetoothLowEnergyNotSupported()) return Constants.BluetoothState.UNSUPPORTED;
        if (bluetoothManager == null) return Constants.BluetoothState.POWERED_OFF;
    ...
    private void safeConnectToDevice2(final RxBleDevice device,
        ...
        Observable<RxBleConnection> connect = device
                ...
                .doOnNext(new Consumer<RxBleConnection>() {
                    @Override
                    public void accept(RxBleConnection connection) throws Exception {
                        Device localDevice = rxBleDeviceToDeviceMapper.map(device);
                        onConnectionStateChangedCallback.onEvent(ConnectionState.CONNECTED);
                        cleanServicesAndCharacteristicsForDevice(localDevice);
